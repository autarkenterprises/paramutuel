...
| `get_wager` | Full wager details: outcomes, ticket pools (enumerated bitmask or freeform answer ids), totals, event history |
...
| `quote_place_bet` | Fetch wager, compute odds, and return `placeBet` calldata + approval details (enumerated mode maps outcome index → `1<<index` for single-bit tickets; freeform takes the UTF-8 answer) |
| `quote_place_bets` | Fetch wager, compute batch odds, and return `placeBets` calldata + approval details (enumerated uses ticket masks per leg) |
...
| Tool | Description |
|------|-------------|
| `encode_create_enumerated_wager` | Create an **enumerated** wager via `ParamutuelFactoryV3.createEnumeratedWager` (bitmask tickets + payoff policy + optional seed ticket masks) |
| `encode_create_freeform_wager` | Create a **freeform** wager via `ParamutuelFactoryV3.createFreeformWager` (UTF-8 answers, no outcome list) |
| `encode_place_bet` | `placeBet(uint256,uint256)` on an enumerated wager (ticket mask + amount) |
| `encode_place_bet_freeform` | `placeBet(string,uint256)` on a freeform wager (exact UTF-8 answer bytes) |
| `encode_place_bets` | Batch `placeBets(uint256[],uint256[])` for enumerated wagers |
| `encode_resolve` | `resolve(uint256)` on an enumerated wager (winning bitmask; resolver only) |
...
| `FACTORY_ADDRESS` | From `config/deployments.json` `factoryAddress` | ParamutuelFactoryV3 contract address (ADR-0010 unified factory for enumerated + freeform wagers) |
...
- **Anyone** can create an **enumerated** wager with 2–255 outcomes + bitmask tickets / payoff policies, or a **freeform** (ADR-0009 / ADR-0010) UTF-8 text-answer wager using any ERC-20 as collateral — both modes share the unified `ParamutuelFactoryV3` / `ParamutuelWagerV3` protocol
