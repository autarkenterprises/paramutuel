...
Almost none of these bets are formalized. The ones that are get shoehorned into prediction market platforms designed for trading, not betting. Polymarket is great for "will X happen?" binary markets with continuous pricing. But it's poorly suited for "let's all throw $20 into a pot and the people who picked the right answer split it" — which is what most real-world prop bets actually look like.

**The parimutuel model** — where participants bet into a shared pool and winners split the pot pro rata — is the natural primitive for this. It's how horse racing has worked for over a century. It needs no market maker, no order book, no liquidity provider. Just a proposition, outcomes, and bettors.

### The solution

An **immutable, permissionless, on-chain protocol** for arbitrary parimutuel proposition betting on Base (Ethereum L2).

- **Anyone can propose a market**: "Will SpaceX land on Mars before 2030?" with outcomes "Yes before 2028 / Yes 2028-2030 / No"
...
- **No house edge, no order book, no market maker**: pure peer-to-peer pool mechanics
- **Resolution is modular**: starts with creator-resolved markets (like Manifold); upgradeable to oracle-backed or DAO-governed resolution without changing the core contracts

### Why now

1. **L2 costs make this viable**: Creating a market on Base costs ~$0.02. Placing a bet costs ~$0.002. The gas cost barrier that killed earlier attempts is gone.
...
3. **AI agents need betting infrastructure**: LLM-powered agents are beginning to manage portfolios and make decisions. A machine-readable betting protocol with deterministic settlement is natural infrastructure for automated knowledge markets.

4. **Prediction markets proved the category, but not the format**: Polymarket's success in 2024-2025 proved massive demand for event-outcome speculation. But the order-book/AMM format excludes casual, multi-outcome, social betting. The parimutuel primitive captures the adjacent demand.
...
|-------|-------------|---------------|
| **Global gambling market** | All legal gambling worldwide | ~$700B+ annually |
...
| **On-chain speculation** | Prediction markets + DeFi options + crypto betting | ~$5-20B annually and growing |
...
**Protocol fee:** 1% of the total pot, charged at market finalization.
...
Revenue accrues to the protocol treasury (governed by a multisig Safe). Additional per-market fees can be configured for creators, resolvers, or other participants (capped at 10% total by the factory contract, target <3% in practice).

**Revenue growth levers:**
- More markets (supply-side: creator adoption, machine integration)
- More bettors per market (demand-side: distribution, UX)
...
- Discord/Telegram/Farcaster bots for in-app market creation
- Protocol-seeded flagship markets on trending topics
...
### Slide 3 — The problem
- Prediction markets are for traders, not bettors
...
- Key stats: 2-64 outcomes, any ERC-20, 1% fee, ~$0.02 to create a market

### Slide 5 — How it works
- Visual walkthrough of market lifecycle
- Emphasis on simplicity: "As easy as posting a poll, but with real stakes"

### Slide 6 — Demo / product screenshots
- dApp market creation flow
- Betting interface with odds preview
- Explorer showing live markets

### Slide 7 — Why parimutuel
- No market maker needed (vs. AMM/CLOB)
...
- L2 costs finally make per-market deployment viable
- Prediction market mania proved demand; we serve the adjacent unmet need
...
The protocol is infrastructure for formalizing propositions and outcomes. Like prediction markets, it serves knowledge discovery and accountability. A bet on whether a policy achieves its stated goal is a knowledge artifact. The protocol's value increases with broader, more diverse participation — the same principle that makes prediction markets epistemically valuable.

### "Why not just use Polymarket?"

Polymarket is an order-book prediction market optimized for binary, high-liquidity, continuously-traded markets. It requires market makers and is poorly suited for casual, multi-outcome, social propositions. A group of friends betting on 5 possible outcomes of a local event is not a Polymarket use case. It is a parimutuel use case.

### "What about regulation?"

The protocol is an immutable, permissionless smart contract on a public blockchain. There are no admin keys, no upgrade paths, and no ability to freeze or censor markets. The protocol layer is analogous to TCP/IP — it is infrastructure. The service entity that proposes and resolves markets operates transparently and may need to comply with applicable regulations, but the protocol itself is a neutral public good. Legal counsel should be engaged before mainnet launch.

### "What if a resolver cheats?"

In the MVP, resolution trust is social — the same model as Manifold Markets. Bettors choose who to trust based on on-chain history. This is already better than informal bets (which have no enforcement at all). The architecture supports pluggable resolution: oracle-backed resolution (Chainlink, UMA), DAO/committee resolution, and optimistic/challenge-based resolution are all achievable by deploying a resolver contract and setting it as the market's resolver address. No protocol upgrade required.

### "Can you handle high volume?"

Each market is an independent contract. There is no shared state between markets, so the protocol scales horizontally with the chain's throughput. On Base at current gas prices, creating 1,000 markets costs ~$20. The bottleneck is demand, not capacity.

### "What about front-running / MEV?"

Parimutuel pools are inherently less vulnerable to front-running than order-book markets because there is no spread to extract. A last-second bet changes the odds for everyone (including the front-runner) proportionally. The main MEV vector is a resolver who bets and then resolves in their favor — which is addressed by reputation, social accountability, and future resolver modules with challenge windows.
...
4. **Traction data**: gather testnet metrics (markets created, bets placed, unique addresses)
