...
- `ADR-0018-collateral-curation-overlay.md`
  - **Proposed** (design ADR). Implements ADR-0006's already-accepted collateral-curation decision (still "copy, not flow code" per its AAR) as an off-chain `config/collateral-allowlist.json` overlay consumed by the dApp / site / indexer, under the standing **"neutral core, safety as opt-in overlay"** principle. **No `src/` change**: the core stays collateral-agnostic and its solvency guarantee is documented as scoped to *standard* ERC-20; no-bool tokens (USDT-class) fail safe by revert, fee-on-transfer / rebasing tokens are pinned out-of-scope by regression tests. The universal-fallback path is preserved behind a warned opt-in so the protocol stays permissionless. Builds on ADR-0006 / ADR-0016 (Tier-1 / Tier-2) / ADR-0017 (manifest). Open questions for operator input on the curated token set per network and whether to later add the optional on-chain registry + curated-factory wrapper.
