...
# Update config/deployments.json factoryV2Address for a target network key.
# Usage:
#   ./script/testnet/set_factory_v2_address.sh 0x... [base-sepolia|base-mainnet]

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT_DIR"

if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "usage: $0 <factory-v2-address> [network-key]" >&2
  exit 1
fi
...
if not re.fullmatch(r"0x[a-fA-F0-9]{40}", address):
    raise SystemExit("error: factory v2 address must be a 20-byte hex address (0x...)")
...
entry["factoryV2Address"] = address
...
print(f"updated {config_path} -> {cfg_key}.factoryV2Address = {address}")
PY
