...
A second test class (`TestBaseSepoliaStressV3PolicyMatrix`) exercises the unified **`ParamutuelFactoryV3`** enumerated mode with the same role separation, covering **every payoff policy**, **2- and 3-outcome** markets, **`placeBet` vs `placeBets`**, and rotating **resolve / retract / expire** branches.
...
### V3 enumerated policy matrix (ADR-0010)

| Variable | Default | Meaning |
|----------|---------|--------|
| `FACTORY_ADDRESS` | from `deployments.json` `factoryAddress` | Unified V3 factory; required for all stress tests |
| `STRESS_SKIP_V3_MATRIX` | (unset) | Set to `1` / `true` to skip the V3 enumerated policy matrix class |
| `STRESS_V3_CASES` | (all) | Comma-separated case ids from `V3_FUNDED_RESOLVE_CASES` (`single_winner`, `any_of`, `exact_set`, `at_least_k`, `weighted_overlap`) |

**Wallet pool sizing:** the V3 policy matrix needs the same per-row budget as the general stress flow — four role keys + one (or two, for `any_of`/`weighted_overlap`) bettor per row. For the default case set, generate at least **25** funded-capable wallets (e.g. `python3 script/testnet/gen_stress_wallet_pool.py 32 …`).
...
- **V3 enumerated policy-matrix funded-tx** multiplies legs per scenario (some policies use two ticket masks or `placeBets`), so bettor collateral requirements scale with `STRESS_BET_AMOUNT` and the enumerated wager count.
